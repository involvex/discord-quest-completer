...
export function useFetchGameList() {
  ...
  const {
    ...
    () => {
      const result = import("../assets/gamelist.json").then(
        (res) => res.default,
      );
  ...
  async function fetchGameList() {
    ...
    try {
      ...
      addLog(
        "error",
        "Error executing fetch for GitHub mirror or bundled game list.",
      );
    }
    ...
    if (fetchError.value) {
      await message(
        "There was an error fetching the latest game list." + fetchError.value,
        {
    ...
    if (
      gameListGHMirror.value &&
      gameListGHMirror.value?.length > 0 &&
      isValidGameList(gameListGHMirror.value)
    ) {
      gameDB.value = (gameListGHMirror.value as Game[]) || [];
      addLog(
        "Using game list from GitHub mirror. " +
          gameListGHMirror.value.length +
          " entries.",
      );
    } else if (
      gameListFromDiscord.value &&
      gameListFromDiscord.value?.length > 0 &&
      isValidGameList(gameListFromDiscord.value)
    ) {
      gameDB.value = (gameListFromDiscord.value as Game[]) || [];
      addLog(
        "Using game list from Discord. " +
          gameListFromDiscord.value.length +
          " entries.",
      );
    } else {
      // bundled is always present.
      addLog(
        "Using bundled game list as fallback." +
          bundledGameList.value.length +
          " entries.",
      );
